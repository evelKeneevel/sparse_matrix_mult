...
template<typename T, size_t ROWS, size_t COLUMNS>
void diagonal_treater<T, ROWS, COLUMNS>::prepare_gpu_data(vector<int> &offset, T * &gpu_diagonals_data,
                                                          uint &data_columns) {
    ...
    for (int times = -int(ROWS) + 1; times < int(COLUMNS); times++) {
        // covers all negative diagonals, and 0 diagonal
    ...
    gpu_diagonals_data = new T[ROWS * data_columns];
    // matrix is not initialized! Will contain garbage, but it's not a problem
    ...
    for (size_t k = 0; k < size_t(data_columns); k++) {
        ...
        if (offset[k] < 0) {
            i = -offset[k];
            j = 0;
            placement_start = -offset[k] * data_columns + k;
        } else {
            i = 0;
            j = offset[k];
            placement_start = k;
        ...
        for (size_t k = j; k < COLUMNS; k++) {
            ...
            gpu_diagonals_data[placement_start] = this->input_array->data()[i * COLUMNS + k];
            // assign values ONLY to cells of interest
...
struct execute_info diagonal_treater<T, ROWS, COLUMNS>::create_gpu_program() {
    ...
    if (!this->created_gpu_data) {
        // creating the GPU data only once
    ...
    cl_context context = clCreateContext(NULL, this->num_devices, &(this->devices[0]), NULL, NULL,
                                         &this->return_status);
    ...
    cl_mem a_mem_obj = clCreateBuffer(context, CL_MEM_READ_ONLY, ROWS * gpu_data_columns * sizeof(T), NULL,
                                      &this->return_status);
    cl_mem b_mem_obj = clCreateBuffer(context, CL_MEM_READ_ONLY, gpu_data_columns * sizeof(int), NULL,
                                      &this->return_status);
    ...
    cl_kernel kernel = clCreateKernel(program, string(this->type_identification() + "_compute").data(),
                                      &this->return_status);
...
diagonal_treater<T, ROWS, COLUMNS>::execute_gpu_program(cl_command_queue &command_queue, cl_kernel &kernel) {
    ...
    cout << "DIA GPU result :";
    for (size_t i = 0; i < ROWS; i++) cout << (*Result)[i] << " ";
    cout << endl;
...
void diagonal_treater<T, ROWS, COLUMNS>::create_partial_result(const array<T, COLUMNS> &multiplicant_array,
    ...
    array<T, ROWS> *partial_result = new array<T, ROWS>;
    for (size_t i = 0; i < ROWS; i++) (*partial_result)[i] = T(0);
            ...
            if ((*diagonal_iter).diagonal_number < 0) {
                (*partial_result)[elem_pos - (*diagonal_iter).diagonal_number] =
                        (this->*(this->multiply_and_add))((std::get<0>(*finger)), multiplicant_array[elem_pos],
                                                          (*partial_result)[
                                                              elem_pos - (*diagonal_iter).diagonal_number]);
            } else {
...
diagonal_treater<T, ROWS, COLUMNS>::threaded_multiplication(const array<T, COLUMNS> &multiplicant_vec) {
    ...
    array<T, ROWS> *result = new array<T, ROWS>;
    for (size_t i = 0; i < ROWS; i++) (*result)[i] = T(0);

    size_t step = this->calculate_step();

    for (size_t i = 0; i < size_t(this->used_threads); i++) {
        size_t finish = ((i + 1) * step < (*this->input_array).size())
                            ? (i + 1) * step - 1
                            : (*this->input_array).size() - 1;
        // If some elements are remaining, they are all placed in the last thread's work
        if (i == size_t(this->used_threads - 1) && finish < (*this->input_array).size() - 1)
            finish = (*this->input_array).size() - 1;
        ...
        thread_vector.push_back(std::thread{
            &diagonal_treater::create_partial_result, this,
            std::ref(multiplicant_vec), start, finish, std::move(promise)
        }); // move the promise into the thread function

        if (finish == (*this->input_array).size() - 1) i = this->used_threads;
        // Break loop, if all elements have been reached
    }
...
diagonal_treater<T, ROWS, COLUMNS>::normal_multiplication(const array<T, COLUMNS> &multiplicant_array) {
    array<T, ROWS> *result = new array<T, ROWS>();
    for (size_t i = 0; i < ROWS; i++) (*result)[i] = 0;
    ...
    for (auto i = diagonals.begin(); i != diagonals.end(); i++) {
        ...
        if ((*i).diagonal_number < 0) { result_store_start -= (*i).diagonal_number; } else {
            multiplicant_start += (*i).diagonal_number;
        }
