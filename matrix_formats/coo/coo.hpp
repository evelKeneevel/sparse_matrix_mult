...
public:
    ...
    ~coo_treater() {
        // delete all allocated matrixes
        if (elem_per_row != nullptr) {
            delete [] elem_per_row;
            elem_per_row = nullptr;
        }
        if (gpu_row_indexes != nullptr) {
            delete [] gpu_row_indexes;
            gpu_row_indexes = nullptr;
        }
        if (gpu_column_indexes != nullptr) {
            delete [] gpu_column_indexes;
            gpu_column_indexes = nullptr;
        }
        if (gpu_data != nullptr) {
            delete [] gpu_data;
            gpu_data = nullptr;
...
struct execute_info coo_treater<T, ROWS, COLUMNS>::create_gpu_program() {
    ...
    if (!this->created_gpu_data) {
        gpu_data = new T[ROWS * COLUMNS];
        gpu_row_indexes = new uint[ROWS * COLUMNS];
        gpu_column_indexes = new uint[ROWS * COLUMNS];
        elem_per_row = new uint[ROWS];
    ...
    cl_mem a_mem_obj = clCreateBuffer(context, CL_MEM_READ_ONLY, nonzero_elements * sizeof(T), NULL,
                                      &this->return_status);
    cl_mem b_mem_obj = clCreateBuffer(context, CL_MEM_READ_ONLY, nonzero_elements * sizeof(uint), NULL,
                                      &this->return_status);
    cl_mem c_mem_obj = clCreateBuffer(context, CL_MEM_READ_ONLY, nonzero_elements * sizeof(uint), NULL,
                                      &this->return_status);
    ...
    this->vals_mem_obj = clCreateBuffer(context, CL_MEM_WRITE_ONLY, nonzero_elements * sizeof(T), NULL,
                                        &this->return_status);
...
coo_treater<T, ROWS, COLUMNS>::execute_gpu_program(cl_command_queue &command_queue, cl_kernel &kernel) {
    ...
    std::array<T, ROWS> *Result = new std::array<T, ROWS>;
...
coo_treater<T, ROWS, COLUMNS>::threaded_multiplication(const array<T, COLUMNS> &multiplicant_vec) {
    ...
    for (size_t i = 0; i < size_t(this->used_threads); i++) {
        ...
        thread_vector.push_back(std::thread{
            &coo_treater::calculate_threaded_result, this, i * step, finish,
...
void coo_treater<T, ROWS, COLUMNS>::calculate_threaded_result(size_t start_index, size_t end_index,
                                                              ...
                                                              std::promise<array<T, ROWS> > &&promise) {
    std::array<T, ROWS> partial_result;
    for (size_t i = 0; i < partial_result.size(); i++) partial_result[i] = T(0);
...
coo_treater<T, ROWS, COLUMNS>::normal_multiplication(const array<T, COLUMNS> &multiplicant_vec) {
    array<T, ROWS> *result = new array<T, ROWS>;
    for (size_t i = 0; i < (*result).size(); i++) (*result)[i] = T(0);
