...
csr_treater<T, ROWS, COLUMNS>::create_gpu_program() {
    ...
    if (!this->created_gpu_data) {
        ...
        my_indices = new uint[ROWS * COLUMNS];
        my_data = new T[ROWS * COLUMNS];
...
csr_treater<T, ROWS, COLUMNS>::create_threaded_result(const array<T, COLUMNS> &multiplicant_vec, size_t start,
    ...
    array<T, ROWS> partial_result;
    for (T &i: partial_result) i = T(0);

    //  array<T, ROWS> *partial_result = new array<T, ROWS>;  for( T &i : *partial_result ) i = T(0);

    array<size_t, ROWS + 1> partial_pointers;
    for (size_t &i: partial_pointers) i = size_t(0);
    ...
    cout << "Partial result is: ";
    for (T &i: partial_result) cout << " " << i;
    cout << endl;
