...
ell_treater<T, ROWS, COLUMNS>::prepare_gpu_data(T * &data, uint * &gpu_indices, uint &K_dim_gpu) {
    if (K_dim == 0) {
        // discover the K dimension, if not provided as an argument
        size_t non_zeros = 0;
        for (size_t i = 0; i < ROWS; i++) {
            for (size_t j = 0; j < COLUMNS; j++)
                if ((*this->input_array)[i * COLUMNS + j] != 0) non_zeros++;

            if (non_zeros == COLUMNS) {
                K_dim_gpu = COLUMNS;
                break;
            } // #elements == COLUMNS are non-zero, so we stop the loop
...
ell_treater<T, ROWS, COLUMNS>::create_gpu_program() {
    ...
    cl_mem b_mem_obj = clCreateBuffer(context, CL_MEM_READ_WRITE, ROWS * K_dim_gpu * sizeof(T), NULL,
                                      &this->return_status);
    cl_mem c_mem_obj = clCreateBuffer(context, CL_MEM_READ_WRITE, ROWS * K_dim_gpu * sizeof(uint), NULL,
                                      &this->return_status);
...
void ell_treater<T, ROWS, COLUMNS>::calculate_threaded_result(size_t start_index, size_t end_index,
    ...
    delete [] my_data;
    delete [] my_indices; // delete thread allocated stuff
