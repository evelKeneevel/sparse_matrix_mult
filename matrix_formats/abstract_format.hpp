...
protected:
    ...
    const T (abstract_format::*multiply_and_add)(const T &, const T &, const T &);

    // function pointer, points to class member functions
...
abstract_format<T, ROWS, COLUMNS>::abstract_format() {
    ...
    try { type_res = type_identification(); } catch (std::exception &) {
        cout << "Unappropriate template type!" << endl;
    }
    ...
    if (type_res == "int" || type_res == "short") multiply_and_add = &abstract_format::int_mult;
             ...
             || type_res == "long" || type_res == "edouble")
        multiply_and_add = &abstract_format::fma_mult;
}
...
string abstract_format<T, ROWS, COLUMNS>::type_identification() {
    ...
    if (typeid(T).name()[0] == 'f') { result = "float"; } else if (typeid(T).name()[0] == 'i') result = "int";
...
abstract_format<T, ROWS, COLUMNS>::multiply_with_gpu(const array<T, COLUMNS> &multiplicant_vec) {
    ...
    if (!created_gpu_device) {
        create_device();
        created_gpu_device = true;
    }
