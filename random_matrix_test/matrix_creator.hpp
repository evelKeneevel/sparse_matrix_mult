...
int compare_arrays(const array<T, ARRAY_ROWS> &correct_res, const array<T, ARRAY_ROWS> &B) {
    ...
    for (size_t i = 0; i < ARRAY_ROWS; i++) {
        ...
        if (correct_res[i] - B[i] != 0) {
            cout << " ****** Wrong result! ****** on index " << i << ": " << correct_res[i] << " & " << B[i] <<
                    " ! error: " << correct_res[i] - B[i] << /*" \n"<<*/ endl;
            wrong_result += 1;
            //	return;
        }
    }
...
T mult_function(const T &arr_val, const T &vec_val, const T &part_res) {
    if (typeid(T).name()[0] == 'f') { return fma(arr_val, vec_val, part_res); } else if (
        typeid(T).name()[0] == 'i') { return arr_val * vec_val + part_res; } else if (
        typeid(T).name()[0] == 'd') { return fma(arr_val, vec_val, part_res); } else if (
        typeid(T).name()[0] == 'l') { return fma(arr_val, vec_val, part_res); } else if (typeid(T).name()[0] == 'e') {
        return fma(arr_val, vec_val, part_res);
...
std::array<T, ARRAY_COLUMNS> * /* && */ vector_creator() {
    ...
    for (size_t i = 0; i < ARRAY_COLUMNS; i++) {
        (*vector)[i] = (T) (rand() % 10) / NUMB_DIV; /* T( ((rand()%10)*NUMB) );*/
        cout << " " << (*vector)[i];
    }
    cout << "\n" << endl;
